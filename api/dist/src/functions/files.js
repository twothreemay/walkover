...
app.http("walkoverFiles", {
    ...
    handler: async (request) => {
        ...
        if (request.method === "GET")
            return { jsonBody: { files: await list("files", `PartitionKey eq '${escapeOData(walkoverId)}'`) } };
        const form = await request.formData();
        const uploaded = form.get("file");
        if (!(uploaded instanceof File) || uploaded.size === 0)
            return { status: 400, jsonBody: { error: "A file is required" } };
        const id = crypto.randomUUID();
        const kind = String(form.get("kind") || inferKind(uploaded.name));
        ...
        await table("files").createEntity(item);
        await history(walkoverId, "file", "uploaded", adminName(request), uploaded.name);
...
app.http("fileById", {
    ...
    handler: async (request) => {
        await ensureTables();
        let metadata;
        for await (const item of table("files").listEntities({ queryOptions: { filter: `RowKey eq '${escapeOData(request.params.id)}'` } })) {
            metadata = item;
            break;
        }
        if (!metadata)
        ...
        if (!session(request) && !(await validShare(request.query.get("share"), String(metadata.projectId))))
            return { status: 403, jsonBody: { error: "File access denied" } };
        const download = await (await filesContainer()).getBlobClient(String(metadata.blobName)).download();
        if (!download.readableStreamBody)
            return { status: 404 };
        return { body: download.readableStreamBody, headers: { "content-type": String(metadata.contentType), "content-length": String(download.contentLength || metadata.size || ""), "cache-control": "private, max-age=300" } };
    }
});
async function validShare(token, projectId) {
    if (!token)
        return false;
    try {
        const share = await table("shares").getEntity("SHARE", token);

...
app.http("projects", {
    ...
    handler: async (request) => {
        ...
        if (!body.placeId || !body.title?.trim())
            return { status: 400, jsonBody: { error: "Place and project title are required" } };
        ...
        await table("projects").createEntity(item);
        await history(body.placeId, "project", "created", adminName(request), item.title);
...
app.http("projectById", {
    ...
    handler: async (request) => {
        ...
        if (denied)
            return denied;
        if (request.method === "GET")
            return { jsonBody: { project: clean(await table("projects").getEntity(request.params.placeId, request.params.id)) } };
        const body = await request.json();

...
app.http("organisations", {
    ...
    handler: async (request) => {
        ...
        if (request.method === "GET")
            return { jsonBody: { organisations: await list("organisations") } };
        const body = await request.json();
        if (!body.name?.trim())
            return { status: 400, jsonBody: { error: "Organisation name is required" } };
        ...
        await table("organisations").createEntity(item);
        await history(id, "organisation", "created", adminName(request), item.name);

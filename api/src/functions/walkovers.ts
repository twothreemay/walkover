...
app.http("walkovers", {
  ...
  handler: async (request): Promise<HttpResponseInit> => {
    ...
    await ensureTables();
    if (request.method === "GET") return { jsonBody: { walkovers: await list("walkovers", `PartitionKey eq '${escapeOData(request.params.projectId)}'`) } };
    const body = await request.json() as { title?: string; surveyDate?: string; surveyType?: string; surveyor?: string; captureMethod?: string; notes?: string };
    if (!body.title?.trim() || !body.surveyDate) return { status: 400, jsonBody: { error: "Walkover title and survey date are required" } };
    ...
    await table("walkovers").createEntity(item); await history(request.params.projectId, "walkover", "created", adminName(request), item.title);

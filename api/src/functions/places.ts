...
app.http("places", {
  ...
  handler: async (request): Promise<HttpResponseInit> => {
    ...
    const body = await request.json() as { organisationId?: string; name?: string; placeType?: string; location?: string; description?: string };
    if (!body.organisationId || !body.name?.trim()) return { status: 400, jsonBody: { error: "Organisation and place name are required" } };
    ...
    await table("places").createEntity(item); await history(id, "place", "created", adminName(request), item.name);
